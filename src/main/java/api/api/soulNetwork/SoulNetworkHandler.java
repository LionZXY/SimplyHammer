...
import api.api.event.ItemBindEvent;
import api.api.event.ItemDrainNetworkEvent;
...
public class SoulNetworkHandler
{
    public static boolean syphonFromNetworkWhileInContainer(ItemStack ist, int damageToBeDone)
    {
    	...
    	api.api.event.ItemDrainInContainerEvent event = new api.api.event.ItemDrainInContainerEvent(ist, ownerName, damageToBeDone);
    ...
    public static int syphonFromNetwork(String ownerName, int damageToBeDone)
    {
        ...
        World world = MinecraftServer.getServer().worldServers[0];
        WayofTime.alchemicalWizardry.api.soulNetwork.LifeEssenceNetwork data = (WayofTime.alchemicalWizardry.api.soulNetwork.LifeEssenceNetwork) world.loadItemData(WayofTime.alchemicalWizardry.api.soulNetwork.LifeEssenceNetwork.class, ownerName);

        if (data == null)
        {
            data = new WayofTime.alchemicalWizardry.api.soulNetwork.LifeEssenceNetwork(ownerName);
            world.setItemData(ownerName, data);
        }
    ...
    {
    	api.api.event.AddToNetworkEvent event = new api.api.event.AddToNetworkEvent(ownerName, addedEssence, maximum);
