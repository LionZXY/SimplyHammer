...
import api.api.event.RitualRunEvent;
import api.api.event.RitualStopEvent;
import api.api.renderer.MRSRenderer;
...
public class Rituals
{
    ...
    public final api.api.rituals.RitualEffect effect;
    ...
    public Rituals(int crystalLevel, int actCost, api.api.rituals.RitualEffect effect, String name, MRSRenderer renderer)
    {
    ...
    public Rituals(int crystalLevel, int actCost, api.api.rituals.RitualEffect effect, String name)
    {
     ...
     */
    public static boolean registerRitual(String key, int crystalLevel, int actCost, api.api.rituals.RitualEffect effect, String name, MRSRenderer renderer)
    {
    ...
    public static boolean registerRitual(String key, int crystalLevel, int actCost, api.api.rituals.RitualEffect effect, String name)
    {
        if (ritualMap.containsKey(key))
        {
            return false;
    ...
    {
        List<api.api.rituals.RitualComponent> ritual = Rituals.getRitualList(ritualID);
        ...
        for (api.api.rituals.RitualComponent rc : ritual)
        {
            ...
            if (!(test instanceof IRitualStone && ((IRitualStone)test).isRuneType(world, x + rc.getX(direction), y, z+ rc.getZ(direction), world.getBlockMetadata(x + rc.getX(direction), y + rc.getY(), z + rc.getZ(direction)), rc.getStoneType()))
                    && !(te instanceof api.api.rituals.ITileRitualStone && ((api.api.rituals.ITileRitualStone)te).isRuneType(rc.getStoneType())))
            {
                return false;
            }
        }
    ...
    public static List<api.api.rituals.RitualComponent> getRitualList(String ritualID)
    {
        if (ritualMap.containsKey(ritualID))
        {
            Rituals ritual = ritualMap.get(ritualID);
    ...
    private List<api.api.rituals.RitualComponent> obtainComponents()
    {
    ...
    public static void performEffect(api.api.rituals.IMasterRitualStone ritualStone, String ritualID)
    {
    ...
    public static boolean startRitual(api.api.rituals.IMasterRitualStone ritualStone, String ritualID, EntityPlayer player)
    {
        if (ritualMap.containsKey(ritualID))
        {
            Rituals ritual = ritualMap.get(ritualID);
    ...
    public static void onRitualBroken(api.api.rituals.IMasterRitualStone ritualStone, String ritualID, api.api.rituals.RitualBreakMethod method)
    {
    	String ownerName = ritualStone.getOwner();
    ...
    public static api.api.rituals.LocalRitualStorage getLocalStorage(String ritualID)
    {
    	if (ritualMap.containsKey(ritualID))
        {
            Rituals ritual = ritualMap.get(ritualID);
            if (ritual != null)
            {
                api.api.rituals.RitualEffect eff = ritual.effect;
                if(eff != null)
                {
