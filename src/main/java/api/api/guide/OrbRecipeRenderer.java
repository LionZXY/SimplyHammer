...
public class OrbRecipeRenderer implements IRecipeRenderer
{
	...
	public void draw(Book book, CategoryAbstract category, EntryAbstract entry,
        ...
        if(recipe instanceof api.api.items.ShapelessBloodOrbRecipe)
        {
        	api.api.items.ShapelessBloodOrbRecipe shapelessBloodOrbRecipe = (api.api.items.ShapelessBloodOrbRecipe) recipe;
                ...
                for (int x = 0; x < width; x++) {
                    ...
                    if (component != null) {
                        if (component instanceof ItemStack) {
                            ...
                            GuiHelper.drawItemStack(api.api.spell.APISpellHelper.getOrbForLevel((Integer) component), stackX, stackY);
                            if (GuiHelper.isMouseBetween(mouseX, mouseY, stackX, stackY, 15, 15)) {
                                guiBase.renderToolTip(api.api.spell.APISpellHelper.getOrbForLevel((Integer) component), stackX, stackY);
                            }
                        } else {
        ...
        {
            api.api.items.ShapedBloodOrbRecipe shapedBloodOrbRecipe = (api.api.items.ShapedBloodOrbRecipe) recipe;
            int width = ReflectionHelper.getPrivateValue(api.api.items.ShapedBloodOrbRecipe.class, shapedBloodOrbRecipe, 4);
            int height = ReflectionHelper.getPrivateValue(api.api.items.ShapedBloodOrbRecipe.class, shapedBloodOrbRecipe, 5);
            for (int y = 0; y < height; y++) {
                for (int x = 0; x < width; x++) {
                    int stackX = (x + 1) * 18 + (guiLeft + guiBase.xSize / 7);
                    int stackY = (y + 1) * 18 + (guiTop + guiBase.ySize / 5);
